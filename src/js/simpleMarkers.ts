...
L.Control.SimpleMarkers = L.Control.extend({
    ...
    enterAddMarkerMode: function () {
        ...
        this.map._container.style.backgroundColor = 'green';
    ...
    enterDelMarkerMode: function () {
        ...
        for (var marker = 0; marker < this.markerList.length; marker++) {
            if (typeof(this.markerList[marker]) !== 'undefined') {
                ...
                this.map._container.style.cursor = 'crosshair';
                this.map._container.style.backgroundColor = 'red';
            }
        }
    },

    onMapClickAddMarker: function (e: any) {
        "use strict";
        this.map.removeEventListener('click');
        this.map._container.style.cursor = 'auto';

        var marker_options = {
            draggable: this.options.marker_draggable,
            // icon: this.options.marker_icon || undefined,
        };
        ...
        this.map.fire('simplemarkers:markeradded', { marker });

        return false;
    },

    onMarkerClickDelete: function (e: any) {
        "use strict";
        this.map._container.style.cursor = 'auto';
        if (this.markerList.indexOf(e.target)) {
            ...
            this.map.fire('simplemarkers:markerremoved', { marker: e.target });
